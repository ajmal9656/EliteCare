  ...
  useEffect(()=>{
    const fetchMedicalRecords = async () => {
      try {
        ...
        const response = await axiosUrl.get(`/doctor/getMedical-records/${appointment.viewDetails.userId._id}`); // Adjust the API endpoint as needed
  ...
  <div className="flex flex-col w-full mx-auto pl-80 p-4 ml-3 mt-20 h-screen px-10 space-y-3">
    <div>
          ...
          <div className='w-[35%] h-[240px] bg-white rounded-lg shadow-md'>
            <div className='h-[200px] w-[90%] flex ml-8 items-center space-x-2'>
              <ul className='space-y-4 mt-10'>
                ...
                <li className='space-x-3 flex'>
                  <h1 className='text-lg font-semibold text-gray-800'>Age:</h1>
                  <p className='text-lg text-gray-700'>{appointment?.viewDetails?.age || 'N/A'}</p>
                </li>
                <li className='space-x-3 flex'>
                  <h1 className='text-lg font-semibold text-gray-800'>Date:</h1>
                ...
                <li className='space-x-3 flex'>
                  <h1 className='text-lg font-semibold text-gray-800'>Fees:</h1>
      ...
      <div className="w-full h-[500px] bg-white rounded-md shadow-xl flex flex-col mt-4">
        <h2 className="text-2xl font-semibold mb-2 p-4 text-gray-800">Medical Records</h2>
          ...
          <table className="min-w-full table-auto">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-gray-700 font-semibold flex justify-end mr-9">Prescription</th>
              </tr>
            </thead>
            <tbody>
              {medicalRecords.map((record: any) => (
                <tr key={record._id} className="border-b border-gray-200">
                  <td className="px-6 py-4 text-gray-700">{record.patientNAme}</td>
                  <td className="px-6 py-4 text-gray-700">{new Date(record.date).toLocaleDateString()}</td>
          ...
          {appointmentStatus === 'cancelled by Dr' ? (
            <>
              ...
              <div className="max-h-60 overflow-y-auto overflow-x-hidden">
                <p className="break-words text-gray-700">{appointment?.viewDetails?.reason}</p>
              </div>
            </>
          ) : (
